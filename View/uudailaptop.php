                <div class="title">
                    <p class="product-selling">
                        Ưu Đãi Laptop
                    </p>
                </div>
                <ul id="autoWidth3" class="cs-hidden3">
                    <!-----1-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-1.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop gaming Acer Nitro 5 AN515 55 5923</a>
                                <div class="price-del">
                                    <del>23,990,000₫</del>
                                    <div class="price">22,290,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----2-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-2.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop Gaming MSI GF63 Thin 9SCSR 846VN</a>
                                <div class="price-del">
                                    <del>24,960,000₫</del>
                                    <div class="price">23,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----3-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-3.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop Gaming MSI GF63 Thin 10SCSR 830VN</a>
                                <div class="price-del">
                                    <del>27,960,000</del>
                                    <div class="price">23,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----4-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-4.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop Gaming MSI GF65 Thin 10SDR 623VN</a>
                                <div class="price-del">
                                    <del>28,960,000₫</del>
                                    <div class="price">25,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>

                    <!-----5-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-5.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop gaming Acer Nitro 5 AN515 55 77P9</a>
                                <div class="price-del">
                                    <del>28,960,000₫</del>
                                    <div class="price">26,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----6-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-6.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop gaming Acer Nitro 5 AN515 55 55E3</a>
                                <div class="price-del">
                                    <del>31,960,000₫</del>
                                    <div class="price">28,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----7-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-7.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop Gaming ASUS ROG Zephyrus G15 GA502IU AL007T
                                </a>
                                <div class="price-del">
                                    <del>29,960,000₫</del>
                                    <div class="price">28,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>
                    <!-----8-->
                    <li class="item-a">
                        <div class="box">
                            <img src="Asset/img/laptop-8.jpg" class="model" alt="n-p1">
                            <div class="detail">
                                <a href="#" class="name-product">Laptop Gaming MSI GL65 Leopard 10SDK 242VN</a>
                                <div class="price-del">
                                    <del>33,960,000₫</del>
                                    <div class="price">30,390,000₫</div>
                                </div>
                            </div>
                        </div>
                    </li>


                </ul>